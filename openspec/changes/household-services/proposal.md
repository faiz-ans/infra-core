...
The site has files, photos, media, passwords, and home automation, but not notes, bookmarks, remote desktop, cameras, or a travel log. The IronWolf is here; these apps should land on the current `DATA_ROOT` before that disk move so they migrate with everything else.
...
- Add **ZoneMinder** on `periphery`: camera NVR. Web UI through Caddy. Event recordings on NFS `shared/cameras`. MariaDB and ZoneMinder config stay on local HTPC volumes.
...
- `zoneminder`: ZoneMinder on periphery, Caddy `cams.` / `zm.` / `zoneminder.`, recordings on `${NFS_EXPORT}/cameras`.
...
- Add `stacks/workload/{jotty,linkding,rustdesk,zoneminder,adventurelog}/`.
...
- First-run: `bootstrap/{jotty,linkding,rustdesk,zoneminder,adventurelog}.md`.
- Runtime: Core containers `jotty`, `linkding`, `hbbs`, `hbbr`; HTPC stacks `zoneminder` and `adventurelog`.
