...
A thin remote bootstrap SHALL start Komodo Periphery (typically as a Docker Desktop container with the engine socket mounted) in outbound mode to Core, with `connect_as` equal to `PERIPHERY_SERVER` (default `periphery`). Core MUST NOT need an inbound connection to the remote OS for agent control. The bootstrap SHALL prompt for values the engine needs (Core address, onboarding key, Docker Desktop-visible `DATA_ROOT` / share path, `BACKUP_DRIVE`) and write them on the box.
