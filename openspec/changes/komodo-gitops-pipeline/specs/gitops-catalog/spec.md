...
ResourceSync TOML SHALL assign stacks to Komodo server names `nas` and `htpc` only (not hostnames or IPs). Stacks SHALL point at this git repo and a `run_directory` inside `stacks/`.
