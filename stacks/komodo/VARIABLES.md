...
`HOMEPAGE_VAR_OMV_USERNAME` / `HOMEPAGE_VAR_OMV_PASSWORD` are the OpenMediaVault **workbench** login (usually user `admin`), not an SSH/Linux account. Names must be exactly those keys (not `OMV_USERNAME`). `$` in the password is fine: Compose must not interpolate that secret (it is env_file-only). After creating them, Redeploy **homepage** so Komodo rewrites `.env` and recreates the container. Repeated failed widget logins can lock the workbench; reset the counter with `omv-firstaid` if the UI also refuses admin.
