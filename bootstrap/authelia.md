...
| OpenCloud: Consent Request every login | Authelia must collect consent for `offline_access` (refresh tokens), so `consent_mode: implicit` is ignored. Catalog uses `pre-configured` for 1y. Redeploy **authelia**, consent once, tick remember. |
