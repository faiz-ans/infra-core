...
| OpenCloud: cannot create Spaces / no admin UI as faiz | Roles come from Authelia `groups`, not the built-in `admin` login. Catalog maps `admins` → OpenCloud admin. Redeploy **opencloud**, then log out and sign in again as **faiz**. |
