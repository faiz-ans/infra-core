...
Caddy is `https://traffic.<DOMAIN>` (`caddymanager.` 301 there). Authelia forward-auth is on the vhost, then CaddyManager’s own login.
...
3. On Core: `sudo bash bootstrap/sync-komodo-secrets.sh` (generates `CADDYMANAGER_JWT_SECRET`).
4. Recreate Komodo Core so `[secrets]` reload.
5. Run `data-root-perms.sh` so `system/caddymanager` exists.

## 2. Deploy

Commit and push. Wait for ResourceSync. Komodo → **Stacks** → **caddymanager** → **Deploy**. Redeploy **caddy** (admin API + vhost) and **homepage**.
