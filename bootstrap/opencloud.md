...
| Collabora iframe white / `unable to get local issuer certificate` | Collabora WOPI-fetches `https://cloud.<DOMAIN>` and must trust Caddy `tls internal`. Browser trust of `office.` is not enough. Redeploy **collabora** after this catalog (entrypoint installs `https://auth.<DOMAIN>/pki/local-root.crt`). From the HTPC: `docker logs collabora` should show `collabora-ca: installed`. Then `docker exec collabora curl -fsS -o /dev/null -w '%{http_code}' https://cloud.<DOMAIN>/` should be 200, not 60. |
