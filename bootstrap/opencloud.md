...
| Collabora Unhealthy | Image probe dials HTTPS unless `extra_params` is one line with `ssl.enable=false`. Wait the 120s start period. `coolwsd --probe --use-env-vars` must exit 0. |
