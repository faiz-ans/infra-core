...
If this site already ran `core.sh` before ByteStash existed, add the keys in `/etc/infra-core/site.env`. Do not re-run bootstrap only for these.

1. Generate a JWT secret: `openssl rand -hex 24`
2. Set **`BYTESTASH_JWT_SECRET`** in `/etc/infra-core/site.env` (and sops attributes).
...
Open **`https://snip.<DOMAIN>`**. Create or keep the local household account as break-glass, then set **`BYTESTASH_ALLOW_NEW_ACCOUNTS`** to `false` in attributes and re-apply (apply.sh / systemd) **bytestash**. Authelia OIDC is on; first Authelia login as **faiz** or **diana** creates a ByteStash user. Elevate **faiz** in ByteStash. Changing `BYTESTASH_JWT_SECRET` later signs everyone out.
