...
| OpenCloud Authelia ok, then “Not logged in / routine safety log out” | Proxy log `dial tcp <NAS_LAN_IP>:443: connection refused`. Core `site` cannot hairpin `NAS_LAN_IP:443`. Catalog maps `auth.<DOMAIN>` → `SITE_HOST_LOOPBACK` (`169.254.1.2`); lan-bind OUTPUT `127.0.0.1:443` → Caddy `:8443`; Authelia client `opencloud` has `claims_policy` (ID-token profile/groups); `PROXY_OIDC_ACCESS_TOKEN_VERIFY_METHOD=none`. Re-run `core-lan-bind.sh --enable`, then re-apply **opencloud**. Mantle OIDC (`immich`, `adventurelog`, …) keeps `hostAliases` → `NAS_LAN_IP` (that is real LAN). |
