...
On the HTPC, allow Windows Firewall TCP **3313** from the LAN (Caddy). See `bootstrap/periphery/README.md`.

```text
docker ps --filter name=transmute --format "table {{.Names}}\t{{.Status}}"
```

You want `transmute` **Up** (healthy after the first minute). The stack reserves the HTPC NVIDIA GPU for FFmpeg NVENC (`bootstrap/periphery/README.md`). A newer Transmute image runs Draw.io Electron `--version` at boot; that crash fills `C:` with WSL dumps. The OIDC entrypoint stubs that probe so the GPU can stay.
