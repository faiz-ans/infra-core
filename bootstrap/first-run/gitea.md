...
Komodo Core is dual-homed: compose `default` is the internet gateway (GitHub); `edge` reaches `gitea:3000`. Caddy/Gitea/Homepage stay on `edge` only. Copy compose as root from `/home/pilot/bootstrap/komodo/compose.yaml` to `/etc/komodo/bootstrap/compose.yaml`, then `cd /etc/komodo/bootstrap` and `docker compose --env-file compose.env -f compose.yaml up -d --force-recreate`. Do **not** `docker network connect` on a running Core. Do not rename the compose `default` network.
