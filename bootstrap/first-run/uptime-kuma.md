...
No attribute for the app. Optional `UPTIME_KUMA_PUSH_URL` is for the restic client (empty until you create a Push monitor).
...
```text
podman ps --filter name=uptime-kuma --format "table {{.Names}}\t{{.Status}}"
```
...
| Type | What | Notes |
|---|---|---|
| HTTP(s) | `https://dash.<DOMAIN>`, `https://cloud.<DOMAIN>`, `https://box.<DOMAIN>`, … | Through Caddy. Expect the internal CA; Kuma may need “Ignore TLS error” for `tls internal` |
