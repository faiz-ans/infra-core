...
Caddy is `https://disks.<DOMAIN>` (`scrutiny.` 301 there). Authelia forward-auth is on the vhost. Homepage scrapes `http://scrutiny:8080` on `edge` (not through Caddy).
...
Commit and push. Wait for ResourceSync. Komodo → **Stacks** → **scrutiny** → **Deploy**. Redeploy **caddy** and **homepage**.
...
| `hdd=0C` on the cage fan | Separate from Scrutiny; see `bootstrap/core-fan.md`. `smartctl -A` on the host |
