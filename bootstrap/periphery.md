...
Set Komodo variable `HTPC_DATA_ROOT` to that Docker-visible path (for example `Z:` with no trailing slash). ResourceSync maps it to compose `${DATA_ROOT}` on periphery stacks. It must not be the laptop internal SSD.
