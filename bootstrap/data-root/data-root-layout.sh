...
#   sudo DATA_ROOT=/srv/dev-disk-by-uuid-… bash bootstrap/data-root/data-root-layout.sh
#
...
if [[ -f "${SCRIPT_DIR}/data-root-defaults.sh" ]]; then
  ...
  echo "  scp bootstrap/data-root/data-root-defaults.sh pilot@192.168.1.110:/tmp/"
  exit 1
fi
