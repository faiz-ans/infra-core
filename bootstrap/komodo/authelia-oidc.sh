...
#   sudo DATA_ROOT=/srv/dev-disk-by-uuid-... DOMAIN=home.lan bash bootstrap/komodo/authelia-oidc.sh
#
...
if docker ps -qf name=^caddy$ | grep -q .; then
  ...
  echo "Caddy is not running; skip caddy-root.crt (dump it later — see bootstrap/first-run/gitea.md)."
fi
...
echo "/etc/komodo/bootstrap/compose.env (see bootstrap/first-run/authelia.md)."
echo
