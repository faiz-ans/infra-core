...
TRANSMUTE_AUTH_SECRET_KEY='$(quote_s "${TRANSMUTE_AUTH_SECRET_KEY}")'
BYTESTASH_JWT_SECRET='$(quote_s "${BYTESTASH_JWT_SECRET}")'
...
if [[ -f "${ANSWERS}" ]]; then
  ...
  if [[ -z "${TRANSMUTE_AUTH_SECRET_KEY:-}" ]]; then
    TRANSMUTE_AUTH_SECRET_KEY=$(rand)
    echo "Generated TRANSMUTE_AUTH_SECRET_KEY (save now; add in Komodo if Core is already running)."
    save_answers
  fi
  if [[ -z "${BYTESTASH_JWT_SECRET:-}" ]]; then
    BYTESTASH_JWT_SECRET=$(rand)
    echo "Generated BYTESTASH_JWT_SECRET (save now; add in Komodo if Core is already running)."
    save_answers
  fi
  if [[ -z "${BYTESTASH_ALLOW_NEW_ACCOUNTS:-}" ]]; then
    BYTESTASH_ALLOW_NEW_ACCOUNTS=true
    save_answers
  fi
  if [[ -z "${OPENREADER_AUTH_SECRET:-}" ]]; then
    OPENREADER_AUTH_SECRET=$(rand)
    echo "Generated OPENREADER_AUTH_SECRET (save now; add in Komodo if Core is already running)."
    save_answers
  fi
  ...
  prompt_secret TRANSMUTE_AUTH_SECRET_KEY "Transmute JWT signing key"
  prompt_secret BYTESTASH_JWT_SECRET "ByteStash JWT secret"
  prompt BYTESTASH_ALLOW_NEW_ACCOUNTS "ByteStash ALLOW_NEW_ACCOUNTS" "true"
  prompt_secret OPENREADER_AUTH_SECRET "OpenReader session secret"
  prompt_secret N8N_ENCRYPTION_KEY "n8n credential encryption key"
  save_answers
fi
...
TRANSMUTE_AUTH_SECRET_KEY = "${TRANSMUTE_AUTH_SECRET_KEY}"
BYTESTASH_JWT_SECRET = "${BYTESTASH_JWT_SECRET}"
BYTESTASH_ALLOW_NEW_ACCOUNTS = "${BYTESTASH_ALLOW_NEW_ACCOUNTS}"
OPENREADER_AUTH_SECRET = "${OPENREADER_AUTH_SECRET}"
