...
```text
powershell -ExecutionPolicy Bypass -File bootstrap/periphery/periphery-docker-engine.ps1
```
...
If `volume rm` hangs, quit Docker Desktop, `wsl --shutdown`, start Docker, re-run the script (or `bootstrap\periphery\htpc-recover.ps1 -NasIp <NAS_LAN_IP>`).
...
| 61208 | Glances (`host2.${DOMAIN}` via Caddy; Homepage host stats) |
| 8971 | Frigate UI (`nvr.${DOMAIN}` via Caddy) |
...
Create `periphery.env` next to `bootstrap/periphery/periphery.compose.yaml`:
