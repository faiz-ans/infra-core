# Pin HTPC_UPSTREAM on the Ethernet NIC. Safe to re-run. Does not restart Docker.
# Do not rely on a router DHCP reservation: that lease is typically the Wi-Fi
# MAC. Ethernet is a different NIC and will not get HTPC_UPSTREAM while the
...
#   powershell -ExecutionPolicy Bypass -File bootstrap\periphery\htpc-lan-static.ps1 -HtpcIp 192.168.1.111 -NasIp 192.168.1.110 -Gateway 192.168.1.1
#
# Run elevated. Plug Ethernet first. Releases HTPC_UPSTREAM from Wi-Fi so the
...
Write-Host "Komodo HTPC_UPSTREAM stays $HtpcIp. If pihole-periphery cannot bind :53, Redeploy that stack."
exit 0
