...
# Prefer fixing root causes via bootstrap/periphery/periphery-docker-engine.ps1 + Core omv-nfs.sh.
# Usage:
#   powershell -ExecutionPolicy Bypass -File bootstrap\periphery\htpc-recover.ps1 -NasIp <NAS_LAN_IP>
param(
...
if (-not $RepoRoot) {
  ...
  $bootstrap = Split-Path -Parent $here
  $catalog = Split-Path -Parent $bootstrap
  if (Test-Path (Join-Path $catalog "stacks")) {
    $RepoRoot = $catalog
  } else {
    $RepoRoot = $bootstrap
  }
}
$enginePs1 = Join-Path $RepoRoot "bootstrap\periphery\periphery-docker-engine.ps1"
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "NFS :/shared failed. On Core: sudo HTPC_IP=<htpc> bash bootstrap/omv/omv-nfs.sh"
