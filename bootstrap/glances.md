...
Runs on **periphery**. Homepage scrapes `http://<HTPC_UPSTREAM>:61208`. On Docker Desktop this is the WSL2 VM (where the stacks run), not the Windows GUI session. Host `/` is overlay in the container (Glances hides it); the catalog bind-mounts it at `/mnt/host` for the disk widget.
...
| Periphery disk missing | Redeploy **glances-periphery** after the catalog pull. Then `docker exec glances-periphery df -h /mnt/host`. Widget path is `/mnt/host`, not `/` |
