...
#
#   sudo DATA_ROOT=… bash bootstrap/opencloud/opencloud-adopt-shared.sh park
#   # browser: Spaces → New Space → name exactly "shared"; add household members
#   sudo DATA_ROOT=… bash bootstrap/opencloud/opencloud-adopt-shared.sh publish
#   sudo DATA_ROOT=… bash bootstrap/opencloud/opencloud-adopt-shared.sh restore
#
# Existing site (space was bound to whole shared/):
#   sudo DATA_ROOT=… bash bootstrap/opencloud/opencloud-adopt-shared.sh narrow
set -euo pipefail
...
if [[ -f "${SCRIPT_DIR}/../data-root/data-root-defaults.sh" ]]; then
  # shellcheck source=../data-root/data-root-defaults.sh
