...
#   sudo DATA_ROOT=/srv/dev-disk-by-uuid-… bash bootstrap/opencloud/opencloud-check.sh
#
# Exit 0 only if all required checks pass. Does not mount, chown, or deploy.
# Greenfield: run after publish + data-root-layout.sh (see bootstrap/first-run/opencloud.md).
...
if command -v systemctl >/dev/null 2>&1; then
  if systemctl is-enabled opencloud-posix-scan.timer >/dev/null 2>&1; then
    ...
    fail "opencloud-posix-scan.timer not enabled (SMB writes will not show in OpenCloud; bootstrap/opencloud/opencloud-posix-scan.sh)"
  fi
fi
...
echo "${FAILS} check(s) failed. See bootstrap/first-run/opencloud.md (greenfield) or adopt utilities."
exit 1
