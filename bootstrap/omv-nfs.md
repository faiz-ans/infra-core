...
Docker Desktop bind-mounts of a Windows SMB (or NFS) drive letter go through virtiofs and break pathing. Prefer **`compose.nfs.yaml`** (Docker NFS volume driver) on a healthy NFS client. **This site’s HTPC** uses **`compose.smb.yaml`** instead: Docker mounts OMV SMB directly (`//NAS_LAN_IP/shared/...`), because Docker Desktop NFS here hangs / returns stale file handles. Do not bind `Z:` as `DATA_ROOT`. Those stacks keep `/config` on a local Docker volume so the apps can start if Core is down (Home Assistant also needs that so `ensure-http` can write `trusted_proxies`). Media, downloads, Immich photo trees, and Frigate recordings stay on the NAS shares. OpenCloud on Core uses local binds of `users/` and `shared/`. Catalog default remains **`compose.yaml`** (`${DATA_ROOT}` binds) for local disk or a host NFS/SMB mount. Pick one transport file per stack in ResourceSync; do not merge them.
