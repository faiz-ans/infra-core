...
```
${DATA_ROOT}/
  system/{authelia,vaultwarden,gitea,pihole,wireguard,restic,opencloud}   # Core bind-mounts only
...
- HTPC `/config` is a local Docker volume. Media, downloads, and Immich photo originals stay on NFS. OpenCloud on Core bind-mounts `users/` and `shared/` locally.
